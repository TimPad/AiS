...
# Убедитесь, что эти файлы лежат в той же папке, что и ваш app.py,
# или укажите правильный относительный путь (например, 'data/fields2.geojson')
...
# --- ИЗМЕНЕНО: Загрузчики файлов убраны ---
# spills_file = st.sidebar.file_uploader(...) # УДАЛЕНО
# ais_file = st.sidebar.file_uploader(...)   # УДАЛЕНО
...
# --- ИЗМЕНЕНО: Функция теперь принимает путь к файлу (file_path) ---
@st.cache_data
...
# --- ИЗМЕНЕНО: Убрано условие if spills_file and ais_file. Код выполняется всегда. ---
...
if spills_gdf is None or vessels_gdf is None or spills_gdf.empty or vessels_gdf.empty:
    ...
    st.stop() # Прекращаем выполнение скрипта, если данные не загружены
...
if candidates_df.empty:
    ...
    with tab1:
        # ... (остальной код аналитики без изменений)
